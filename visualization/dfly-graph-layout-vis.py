...
numIter = 2000    #Number of iterations for force directed and spring layouts
...
if systemSize == 150:
    ...
    numLocal = 2           # Number of levels/layers of switches in the dragonfly
    numGlobal = 5           # Number of planes/rails in sumulation (currently only support one)
    numTerminals = 3       # Number of terminals/modelnet_dragonfly's per repetition (from network config file)
    k = numLocal + numGlobal + numTerminals     # Switch radix
else:
    filePath = '/home/noah/Dropbox/RPI/Research/Networks/codes-nemo-results/dfly-3072nodes-adaptive-4000end-heterogeneous-trace-cr10-bkgnd-hf-10mintvl-1000000tintvl-3072ranks-10000mpt-8Bszmsg/'
...
f = open(filePath + 'dragonfly-config-up-connections', 'rb')
data = csv.reader(f)
for row in data:
    up = row
